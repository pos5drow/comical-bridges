...
class NhentaiBridge extends BridgeBase {
  ...
  async getSeriesDetails(seriesId) {
    ...
    if (groups.length)
      tagGroups.push({ label: "Groups", tags: groups });
  ...
  async isFavorite(seriesId) {
    this.requireKey();
    const data2 = await this.getJson(`${BASE}/favorites?page=1`);
    const totalPages = data2.num_pages ?? 1;
    if ((data2.result ?? []).some((item) => String(item.id) === seriesId))
      return true;
    for (let page = 2;page <= totalPages; page++) {
      const p = await this.getJson(`${BASE}/favorites?page=${page}`);
      if ((p.result ?? []).some((item) => String(item.id) === seriesId))
        return true;
    }
    return false;
  }
...
//# debugId=EB228C69CDFC9B0964756E2164756E21
