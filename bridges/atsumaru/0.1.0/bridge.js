...
var pageSchema = exports_external.object({
  ...
  imageUrl: exports_external.string().url(),
...
var filterSchema = exports_external.discriminatedUnion("type", [
  ...
  exports_external.object({ type: exports_external.literal("multiselect"), key: exports_external.string(), label: exports_external.string(), options: exports_external.array(optionSchema) })
]);
var filterValueSchema = exports_external.object({
  key: exports_external.string(),
  value: exports_external.union([exports_external.string(), exports_external.array(exports_external.string()), exports_external.number(), exports_external.boolean()])
});
var sortOptionSchema = exports_external.object({ key: exports_external.string(), label: exports_external.string() });
...
class AtsumaruBridge extends BridgeBase {
  ...
  async getSearchResults(query, page, options) {
    ...
    for (const f of options?.filters ?? []) {
      ...
      if (f.key === "genre" && arr.length)
        clauses.push(arr.map((id) => `genreIds:=\`${id}\``).join(" && "));
  ...
  async getSeriesDetails(seriesId) {
    ...
    if (tagNames.length > 0)
      info.tagGroups = [{ label: "Tags", kind: "theme", tags: tagNames }];
